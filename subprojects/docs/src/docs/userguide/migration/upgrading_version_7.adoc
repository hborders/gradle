...
==== Task types without explicit cacheability annotations

All task types must now be annotated with either `@CacheableTask`, `@DisableCachingByDefault` or `@UntrackedTask`.
Having neither annotation present results in an error.
