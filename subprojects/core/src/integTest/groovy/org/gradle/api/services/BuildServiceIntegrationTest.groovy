...
class BuildServiceIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "does not nag when service is annotated with @ServiceReference and feature preview is enabled"() {
        given:
        serviceImplementation()
        customTaskUsingServiceViaProperty("@${ServiceReference.class.name}(name = 'counterService')")
        buildFile """
    ...
    def "injects shared build service by name when reference is annotated with @ServiceReference(name='...') and feature preview is enabled"() {
        given:
        serviceImplementation()
